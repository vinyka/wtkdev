...
async function handleSendMessage(job) {
  try {
    ...
    logger.error({ error: String(e.message) }, "MessageQueue -> SendMessage: error");
    throw e;
  }
}
...
async function handleVerifySchedules(job) {
  try {
    ...
    Sentry.captureException(e);
    logger.error({ error: String(e.message) }, "SendScheduledMessage -> Verify: error");
    throw e;
  }
}
...
async function handleSendScheduledMessage(job) {
  ...
  try {
    ...
    logger.error({ error: String(e.message) }, "SendScheduledMessage -> SendMessage: error");
    throw e;
  }
}
...
async function handleResumeTicketsOutOfHour(job) {
  ...
  try {
    ...
    Sentry.captureException(e);
    logger.error({ error: String(e.message) }, "SearchForQueue -> VerifyQueue: error");
    throw e;
  }
};
...
async function handleRandomUser() {
  ...
  const jobR = new CronJob('0 */2 * * * *', async () => {
    ...
    try {
          ...
          c.queues.map(async q => {
            ...
            const findUserById = async (userId, companyId) => {
              try {
                ...
                logger.error({ error: String(errorV.message) }, "SearchForUsersRandom -> VerifyUsersRandom: error");
                throw errorV;
              }
            };
      ...
      Sentry.captureException(e);
      logger.error({ error: String(e.message) }, "SearchForUsersRandom -> VerifyUsersRandom: error");
      throw e;
    }
  ...
  const job = new CronJob('*/1 * * * *', async () => {
    ...
    companies.map(async c => {
      ...
      try {
        ...
        Sentry.captureException(e);
        logger.error({ error: String(e.message) }, "Process Lanes -> Verify: error");
        throw e;
      }
  ...
  const job = new CronJob('*/1 * * * *', async () => {
    ...
    companies.map(async c => {
      ...
      try {
        ...
        Sentry.captureException(e);
        logger.error({ error: String(e.message) }, "ClosedAllOpenTickets -> Verify: error");
        throw e;
      }
