...
import { performanceMonitor, trackOperation } from "../PerformanceMonitoringService";
...
import { JidLidMapper, normalizeJid, getJidLidMapping } from "../../helpers/JidLidMapper";
import MediaErrorHandler, { 
  MediaErrorCode, 
  MediaType as MediaErrorType, 
  MediaRecoveryStrategy,
  MediaRetryConfig 
} from "../../helpers/MediaErrorHandler";
...
// Enhanced reaction message processing for Baileys 6.7.19
const getEnhancedReactionMessage = (msg: proto.IWebMessageInfo): string => {
  try {
    ...
    // Enhanced reaction processing with new features
    let reactionText = reaction.text || "👍"; // Default reaction emoji
    
    // Handle reaction removal (new in 6.7.19)
    if (reaction.text === null || reaction.text === "") {
      reactionText = "❌ Reação removida";
    }
    
    // Add reaction metadata if available
    let reactionInfo = `${reactionText}`;
    
    // Include reaction key information for better tracking
    if (reaction.key) {
      const targetMessageId = reaction.key.id;
      const targetFromMe = reaction.key.fromMe;
      
      reactionInfo += ` (em resposta a ${targetFromMe ? 'sua mensagem' : 'mensagem'})`;
      
      // Enhanced reaction processing logging
      const context: LogContext = {
        messageId: msg.key?.id,
        operation: 'reaction_processing',
        metadata: {
          reactionText,
          targetMessageId,
          targetFromMe,
      ...
      messageLogger.logMessageEvent(
        MessageEventType.REACTION_PROCESSED,
        context,
        'Enhanced reaction processed successfully'
      );

      logger.info(`Enhanced reaction processed: ${reactionText} for message ${targetMessageId}`);
    }
    ...
    const errorContext: LogContext = {
      messageId: msg.key?.id,
      operation: 'reaction_processing_error'
    };

    messageLogger.logError(
      error as Error,
      errorContext,
      'Error processing enhanced reaction message'
    );

    logger.error(`Error processing enhanced reaction message ${msg.key?.id}: ${error.message}`);
    return "reaction";
  }
};

// Enhanced interactive message processing for Baileys 6.7.19
const getBodyButton = (msg: any): string => {
  try {
    // Handle traditional buttons message
    if (msg?.messageType === "buttonsMessage" || msg?.message?.buttonsMessage?.contentText) {
      ...
      if (msg.message?.buttonsMessage?.buttons) {
        for (const button of msg.message.buttonsMessage.buttons) {
          bodyMessage += `*${button.buttonId}* - ${button.buttonText?.displayText || button.buttonText}\n`;
        }
      }
    ...
    // Handle enhanced interactive messages (new in 6.7.19)
    if (msg?.message?.interactiveMessage) {
      const interactive = msg.message.interactiveMessage;
      let bodyMessage = `[INTERACTIVE]\n\n`;
      
      // Add header if present
      if (interactive.header?.title) {
        bodyMessage += `*${interactive.header.title}*\n\n`;
      }
      
      // Add body text
      if (interactive.body?.text) {
        bodyMessage += `${interactive.body.text}\n\n`;
      }
      
      // Handle different interactive types
      if (interactive.nativeFlowMessage) {
        // Native flow messages (new format)
        bodyMessage += `[NATIVE_FLOW]\n`;
        if (interactive.nativeFlowMessage.buttons) {
          for (const button of interactive.nativeFlowMessage.buttons) {
            bodyMessage += `*${button.name}* - ${button.buttonParamsJson ? JSON.parse(button.buttonParamsJson).display_text : button.name}\n`;
          }
        }
      } else if (interactive.buttonParamsJson) {
        // Button parameters in JSON format
        try {
          const buttonParams = JSON.parse(interactive.buttonParamsJson);
          if (buttonParams.buttons) {
            for (const button of buttonParams.buttons) {
              bodyMessage += `*${button.id}* - ${button.display_text}\n`;
            }
          }
        } catch (parseError) {
          logger.warn(`Failed to parse button parameters JSON: ${parseError.message}`);
        }
      }
    ...
    // Handle traditional list message
    if (msg?.messageType === "listMessage" || msg?.message?.listMessage?.description) {
      ...
      if (msg.message?.listMessage?.sections) {
        for (const section of msg.message.listMessage.sections) {
          if (section.title) {
            bodyMessage += `**${section.title}**\n`;
          }
          
          if (section.rows) {
            for (const row of section.rows) {
              bodyMessage += `${row.title}`;
              if (row.description) {
                bodyMessage += ` - ${row.description}`;
              }
              bodyMessage += `\n`;
            }
          }
    ...
    // Handle enhanced list messages (new in 6.7.19)
    if (msg?.message?.listMessage?.listType) {
      const listMsg = msg.message.listMessage;
      let bodyMessage = `[ENHANCED_LIST]\n\n`;
      
      if (listMsg.title) {
        bodyMessage += `*${listMsg.title}*\n\n`;
      }
      
      if (listMsg.description) {
        bodyMessage += `${listMsg.description}\n\n`;
      }
      ...
      if (listMsg.buttonText) {
        bodyMessage += `[${listMsg.buttonText}]`;
      }
      
      return bodyMessage;
    }

    // Handle button response messages (when user clicks a button)
    if (msg?.message?.buttonsResponseMessage) {
      const response = msg.message.buttonsResponseMessage;
      return `[BUTTON_RESPONSE] ${response.selectedDisplayText || response.selectedButtonId}`;
    }

    // Handle list response messages (when user selects from list)
    if (msg?.message?.listResponseMessage) {
      const response = msg.message.listResponseMessage;
      let responseText = `[LIST_RESPONSE]`;
      
      if (response.title) {
        responseText += ` ${response.title}`;
      }
    ...
    if (msg?.message?.templateButtonReplyMessage) {
      const template = msg.message.templateButtonReplyMessage;
      return `[TEMPLATE_BUTTON] ${template.selectedDisplayText || template.selectedId}`;
    }

  } catch (error) {
    const errorContext: LogContext = {
      messageId: msg.key?.id,
      operation: 'interactive_message_processing_error',
      metadata: {
    ...
    logger.error(`Error processing interactive message ${msg.key?.id}: ${error.message}`);
  }
...
export const getBodyMessage = (msg: proto.IWebMessageInfo): string | null => {
  try {
    ...
    const types = {
      ...
      reactionMessage: getEnhancedReactionMessage(msg),
...
const getMeSocket = (wbot: Session): IMe => {
  return {
    id: normalizeJid((wbot as WASocket).user.id),
...
const getSenderMessage = (
  ...
  // Use enhanced JID normalization with mapping support
  return senderId ? normalizeJid(senderId) : '';
};
...
const getContactMessage = async (msg: proto.IWebMessageInfo, wbot: Session) => {
  ...
  // Use enhanced JID normalization for better identification
...
// Enhanced media download with retry mechanism and better error handling
const downloadMedia = async (msg: proto.IWebMessageInfo, isImported: Date = null, wbot: Session, maxRetries: number = 3) => {
  // Handle sticker message URL optimization
  ...
  // Get media type with enhanced support for new formats
  const mineType = getMediaType(msg);
  const messageId = msg.key?.id || 'unknown';
  
  if (!mineType) {
    logger.warn(`No media type found in message ${messageId}`);
    return null;
  }

  // Determine media type for enhanced error handling
  const mediaType = MediaErrorHandler.determineMediaType(
    msg, 
    typeof mineType === 'object' && 'mimetype' in mineType ? mineType.mimetype : undefined
  );

  let filename = getMediaFilename(msg, mineType);
  let buffer: Buffer | null = null;

  // Initialize media processing state
  MediaErrorHandler.setMediaProcessingInProgress(messageId, true);

  // Enhanced retry mechanism with media-specific error handling
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    ...
    try {
      const context: LogContext = {
        messageId,
        operation: 'media_download_attempt',
        metadata: {
          attempt,
          maxRetries,
          mediaType: mediaType,
          mimeType: typeof mineType === 'object' && 'mimetype' in mineType ? mineType.mimetype : 'unknown',
      ...
      messageLogger.logMessageEvent(
        ...
        context,
        `Attempting media download (attempt ${attempt}/${maxRetries})`
      );

      logger.info(`Attempting media download (attempt ${attempt}/${maxRetries}) for message ${messageId}, type: ${mediaType}`);

      // Use enhanced download options for Baileys 6.7.19
      ...
      if (buffer && buffer.length > 0) {
        ...
        const successContext: LogContext = {
          messageId,
          operation: 'media_download_success',
          duration: downloadDuration,
          metadata: {
            bufferSize: buffer.length,
            attempt,
            mediaType,
            downloadSpeed: buffer.length / (downloadDuration / 1000), // bytes per second
            filename
          }
        };
        ...
        // Update processing state for success
        MediaErrorHandler.updateMediaProcessingState(messageId, mediaType, MediaErrorCode.DOWNLOAD_FAILED, true);
        MediaErrorHandler.setMediaProcessingInProgress(messageId, false);

        logger.info(`Media download successful for message ${messageId}, size: ${buffer.length} bytes, attempt: ${attempt}`);
        break;
      } else {
      ...
      // Analyze error using enhanced media error handler
      const errorInfo = MediaErrorHandler.analyzeMediaError(err, mediaType);
      ...
      MediaErrorHandler.logMediaError(
        err,
        errorInfo,
        errorContext,
        {
          fileName: filename,
          mimeType: typeof mineType === 'object' && 'mimetype' in mineType ? mineType.mimetype : undefined
        }
      );

      // Log performance metrics for failed attempts
      performanceLogger.logPerformanceMetric({
        operation: 'media_download',
        duration: downloadDuration,
        success: false,
      ...
      // Update processing state for failure
      MediaErrorHandler.updateMediaProcessingState(messageId, mediaType, errorInfo.code);

      // Check if we should continue retrying
      const shouldRetry = MediaErrorHandler.isMediaRetryable(messageId, errorInfo) && !isLastAttempt;
      
      if (!shouldRetry) {
        MediaErrorHandler.setMediaProcessingInProgress(messageId, false);
        
        if (isImported) {
          logger.warn(`Failed to download imported message media - content may no longer be available for message ${messageId}`);
          MediaErrorHandler.logMediaRecoveryFailure(messageId, errorInfo.code, mediaType, errorContext);
          return null;
        }
        
        // Handle different recovery strategies
        switch (errorInfo.recoveryStrategy) {
          case MediaRecoveryStrategy.FALLBACK_TO_PLACEHOLDER:
            logger.info(`Creating fallback placeholder for message ${messageId} due to ${errorInfo.code}`);
            const fallback = MediaErrorHandler.createMediaFallback(mediaType, errorInfo);
            MediaErrorHandler.logMediaRecoverySuccess(messageId, errorInfo.code, mediaType, errorContext);
            return fallback;
            
          case MediaRecoveryStrategy.SKIP_MEDIA:
            logger.info(`Skipping media for message ${messageId} due to ${errorInfo.code}`);
            MediaErrorHandler.logMediaRecoveryFailure(messageId, errorInfo.code, mediaType, errorContext);
            return null;
            
          case MediaRecoveryStrategy.MANUAL_INTERVENTION:
            logger.error(`Manual intervention required for message ${messageId}: ${errorInfo.code}`);
            MediaErrorHandler.logMediaRecoveryFailure(messageId, errorInfo.code, mediaType, errorContext);
            return null;
            
          default:
            logger.error(`All media download attempts failed for message ${messageId}: ${err.message}`);
            MediaErrorHandler.logMediaRecoveryFailure(messageId, errorInfo.code, mediaType, errorContext);
            return null;
        }
      }

      // Calculate media-specific retry delay
      const retryConfig: MediaRetryConfig = {
        attempt,
      ...
      const delay = MediaErrorHandler.calculateMediaRetryDelay(retryConfig);
      
      MediaErrorHandler.logMediaRetryAttempt(messageId, errorInfo, attempt, delay, errorContext);
      
      logger.info(`Retrying media download in ${delay}ms for message ${messageId} (${errorInfo.code})`);
  ...
  const media = {
    data: buffer,
    mimetype: typeof mineType === 'object' && 'mimetype' in mineType ? mineType.mimetype : 'application/octet-stream',
    filename
  };
...
// Enhanced media type detection with support for new formats in Baileys 6.7.19
const getMediaType = (msg: proto.IWebMessageInfo) => {
  return (
    msg.message?.imageMessage ||
    msg.message?.audioMessage ||
    msg.message?.videoMessage ||
    ...
    msg.message?.interactiveMessage?.header?.documentMessage ||
    msg.message?.interactiveMessage?.header?.videoMessage ||
    // New formats in 6.7.19
    msg.message?.ptvMessage || // PTV (Profile TV) messages
    msg.message?.newsletterAdminInviteMessage?.jpegThumbnail ||
    msg.message?.groupInviteMessage?.jpegThumbnail ||
    msg.message?.callLogMesssage
  );
};

// Enhanced filename generation with better extension handling
const getMediaFilename = (msg: proto.IWebMessageInfo, mineType: any): string => {
  let filename = msg.message?.documentMessage?.fileName || "";
  
  if (!filename) {
    // Enhanced extension detection for new formats
    let ext = "bin"; // default extension
    
    if (mineType.mimetype) {
      const mimeTypeParts = mineType.mimetype.split("/");
      if (mimeTypeParts.length > 1) {
        ext = mimeTypeParts[1].split(";")[0];
        
        // Handle special cases for new formats
        if (ext === "ogg" && mineType.mimetype.includes("opus")) {
          ext = "ogg";
        } else if (ext === "webp" && msg.message?.stickerMessage) {
          ext = "webp";
        } else if (ext === "mp4" && msg.message?.ptvMessage) {
          ext = "mp4"; // PTV messages are MP4 format
        }
      }
    }
...
const handleMessage = async (
  ...
  // Define context for performance tracking
  const context: LogContext = {
    whatsappId: wbot.id,
    companyId,
    messageId: msg.key?.id,
  ...
  try {
    ...
    if (isGroup) {
      // Use enhanced group handler service for optimized group processing
      const groupHandlerService = GroupHandlerService.getInstance();
      const groupResult = await groupHandlerService.processGroupMessage(wbot, msg, companyId);
      groupContact = groupResult.groupContact;
      isGroupAdmin = groupResult.isGroupAdmin;
      
      if (!groupContact) {
        logger.warn(`Failed to process group message for ${msg.key.remoteJid}`);
        return;
      }
    }
...
const wbotMessageListener = (wbot: Session, companyId: number): void => {
  wbot.ev.on("messages.upsert", async (messageUpsert: ImessageUpsert) => {
    ...
    logger.debug(`Processing ${messageCount} messages for company ${companyId}`);
    ...
    messages.forEach(async (message: proto.IWebMessageInfo) => {
      ...
      if (messageProcessTime > 1000) { // Log slow message processing
        logger.warn(`Slow message processing: ${messageProcessTime}ms for message ${message.key.id}`);
      }
    });
  ...
  wbot.ev.on("contacts.update", (contacts: any) => {
    ...
    logger.debug(`Completed processing message batch for company ${companyId}`);
  })
  wbot.ev.on("groups.update", async (groupUpdate: GroupMetadata[]) => {
    ...
    try {
      // Use enhanced group handler service for optimized group updates processing
      const groupHandlerService = GroupHandlerService.getInstance();
      await groupHandlerService.handleGroupUpdates(wbot, groupUpdate, companyId);
  ...
  // Enhanced group participant events handling with Baileys 6.7.19 optimizations
  wbot.ev.on("group-participants.update", async (participantUpdate: any) => {
    if (!participantUpdate?.id) return;
    
    try {
      const groupParticipantService = GroupParticipantService.getInstance();
      await groupParticipantService.handleGroupParticipantsUpdate(wbot, participantUpdate);
